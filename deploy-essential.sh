...
required_commands=("git" "node" "pnpm" "python3" "pip")
missing_commands=()
...
# Check if ctrlplane and ctrlc-cli are already deployed
if [ ! -d "$ZEEEEPA_BASE_DIR/ctrlplane" ] || [ ! -d "$ZEEEEPA_BASE_DIR/ctrlc-cli" ]; then
    echo -e "${YELLOW}Warning: ctrlplane and/or ctrlc-cli do not appear to be deployed.${NC}"
    echo -e "${YELLOW}This script assumes these components are already deployed.${NC}"
    if ! confirm_continue "Continue with deploying the remaining components?"; then
...
if [ ! -f "$ZEEEEPA_BASE_DIR/stack.sh" ]; then
